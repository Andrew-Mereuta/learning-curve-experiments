...
def get_mandatory_preprocessing(X, y, binarize_sparse=False, drop_first=True):
    ...
    if len(categorical_features) > 0 or sum(missing_values_per_feature) > 0:
        ...
        categorical_transformer = Pipeline([
            ...
            ("binarizer",
             sklearn.preprocessing.OneHotEncoder(drop='first' if drop_first else None, handle_unknown=handle_unknown,
...
def compute_result(openmlid: int, outer_seed: int, inner_seed: int, train_size: int, monotonic: bool, learner,
                   logger=None):
    # load data
    ...
    # get data
    X_train, X_valid, X_test, y_train, y_valid, y_test = get_splits_for_anchor(X, y, train_size, outer_seed, inner_seed,
                                                                               monotonic)
...
def read_classification_file():
    configs = pd.read_csv('../lcdb_classification.csv')
